...
def predict(img):
  ...
  # 影像分割
  raw = np.array(cv2.resize(top2[0], Resize, interpolation = cv2.INTER_AREA))
  sobel_img = np.array(cv2.resize(top2[1], Resize, interpolation = cv2.INTER_AREA))
  top_mask = do_seg(raw, sobel_img)
  top_mask = cv2.flip(top_mask, 0)
  raw = np.array(cv2.resize(bot2[0], Resize, interpolation = cv2.INTER_AREA))
  sobel_img = np.array(cv2.resize(bot2[1], Resize, interpolation = cv2.INTER_AREA))
  bot_mask = do_seg(raw, sobel_img)
